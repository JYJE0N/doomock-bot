...
class DoomockBot {
  ...
  async initializeTelegramBot() {
    ...
    try {
      ...
      Logger.success(`✅ 봇 연결 완료: @${botInfo.username} (${botInfo.first_name})`);
  ...
  async handleCallbackQuery(callbackQuery) {
    ...
    try {
      ...
      const handled = await this.moduleManager.routeCallback(this.bot, callbackQuery, null);
...
아래 버튼을 눌러 원하는 기능을 선택하세요!
    ...
    try {
      ...
      await this.bot.sendMessage(chatId, simpleText, { reply_markup: keyboard });
    }
  }
...
**전체 상태: ${this.getStatusEmoji(healthSummary.status)} ${healthSummary.status.toUpperCase()}**
...
• 건강한 모듈: ${Object.values(moduleHealth.checks.modules).filter(m => m.healthy).length}개
• 문제 모듈: ${Object.values(moduleHealth.checks.modules).filter(m => !m.healthy).length}개

**🗄️ 데이터베이스:**
• 연결 상태: ${dbHealth ? '✅ 정상' : '❌ 문제'}

**⚠️ 감지된 문제:**
${healthSummary.issues.length > 0 ? healthSummary.issues.map(issue => `• ${issue}`).join('\n') : '• 문제 없음'}
...
**🔝 에러 유형 TOP 3:**
${Object.entries(errorStats.byType).slice(0, 3).map(([type, count], index) => 
  `${index + 1}. ${type}: ${count}회`).join('\n') || '• 에러 없음'}

**🔧 모듈별 에러:**
${Object.entries(errorStats.byModule).slice(0, 3).map(([module, count], index) => 
  `${index + 1}. ${module}: ${count}회`).join('\n') || '• 에러 없음'}
    `.trim();
  ...
  async updateUserInfo(user) {
    try {
      ...
      await mongoPoolManager.updateOne(
        'users',
        { userId: user.id },
        userData,
        { upsert: true }
  ...
  // 🔐 관리자 권한 확인
  isAdminUser(userId) {
    const adminUsers = process.env.ADMIN_USERS?.split(',').map(id => parseInt(id)) || [];
