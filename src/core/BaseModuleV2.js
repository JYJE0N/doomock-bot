...
class BaseModuleV2 {
  ...
  async initialize() {
    ...
    try {
      ...
      logger.success(
        `✅ ${this.moduleName}V2 초기화 완료 (${modeText}, EventBus 기반)`
      );
  ...
  async connectService() {
    ...
    try {
      ...
      if (this.service) {
        ...
        logger.warn(
          `⚠️ ${this.moduleName}Service 연결 실패 - 테스트 모드로 동작`
        );
      }
    } catch (error) {
      logger.warn(
  ...
  setupEventListeners() {
    throw new Error(
      `${this.moduleName}V2: setupEventListeners() must be implemented`
    );
  }
  ...
  cleanupExpiredStates() {
    ...
    if (expired.length > 0) {
      ...
      logger.debug(
        `🧹 ${this.moduleName}V2: ${expired.length}개 만료 상태 정리`
      );
    }
  }
