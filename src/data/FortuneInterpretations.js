...
const kph = KoreanPostpositionHelper; // 축약형
...
// 🔮 트리플 카드 해석 템플릿 (함수로 변경)
const TRIPLE_SPREAD_INTERPRETATIONS = {
  temporal: {
    past_influence: [
      (cardName) =>
        `과거의 ${kph.a(cardName, "이/가")} 현재 상황의 씨앗이 되었습니다.`,
      (cardName) => `${kph.a(cardName, "의")} 경험이 당신을 성장시켰습니다.`,
      (cardName) => `지나간 ${kph.a(cardName, "의")} 교훈을 기억하세요.`
    ],
    present_situation: [
      (cardName) =>
        `현재 ${kph.a(cardName, "의")} 에너지가 강하게 작용하고 있습니다.`,
      (cardName) => `${kph.a(cardName, "이/가")} 보여주는 현실을 직시하세요.`,
      (cardName) => `지금은 ${kph.a(cardName, "의")} 지혜가 필요한 때입니다.`
    ],
    future_potential: [
      (cardName) => `미래에는 ${kph.a(cardName, "의")} 결실을 맺을 것입니다.`,
      (cardName) => `${kph.a(cardName, "이/가")} 암시하는 미래를 준비하세요.`,
      (cardName) => `앞으로 ${kph.a(cardName, "의")} 기회가 찾아올 것입니다.`
    ]
  },
  ...
  story_templates: [
    {
      type: "hero_journey",
      template: (data) =>
        `당신은 ${kph.a(data.present, "의")} 상황에서 ${kph.a(data.challenge, "이라는/라는")} 도전에 직면해 있습니다. ${kph.a(data.past, "의")} 경험이 힘이 되어주고, ${kph.a(data.approach, "의")} 자세로 나아간다면, ${kph.a(data.outcome, "의")} 결과를 얻을 것입니다.`
    },
    {
      type: "transformation",
      template: (data) =>
        `${kph.a(data.past, "에서")} 시작된 변화가 ${kph.a(data.present, "에서")} 정점에 달했습니다. ${kph.a(data.challenge, "을/를")} 극복하는 과정에서 ${kph.a(data.hopes_fears, "와/과")} 마주하게 되지만, 궁극적으로 ${kph.a(data.outcome, "으로/로")} 이어질 것입니다.`
    }
  ]
};
