...
class MenuManager {
    ...
    async buildMainMenuItems() {
        ...
        // 화성/동탄 특화 아이템 최우선 배치
        const dongtanItems = menuItems.filter(item => 
    ...
    async showMainMenu(bot, callbackQuery) {
        try {
            ...
            await this.sendErrorMenu(bot, callbackQuery.message.chat.id, '메뉴를 불러올 수 없습니다.');
        }
    }
    ...
    isModuleEnabled(moduleKey) {
        ...
        const isModuleLoaded = this.moduleManager.isModuleLoaded(`${moduleKey.charAt(0).toUpperCase()}${moduleKey.slice(1)}Module`);
    ...
    getContextMenu(context, options = {}) {
        try {
            ...
            switch (context) {
                ...
                case 'confirmation':
                    menuItems = [
                        { text: '✅ 확인', callback_data: options.confirmCallback || 'confirm' },
                        { text: '❌ 취소', callback_data: options.cancelCallback || 'cancel' }
                    ];
                    break;
    ...
    getMenuStats() {
        return {
            ...
            enabledModules: Object.keys(MenuConfig.getMainMenuConfig())
                .filter(key => this.isModuleEnabled(key)).length,
