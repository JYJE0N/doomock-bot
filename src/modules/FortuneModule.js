...
class FortuneModule extends BaseModule {
    ...
    async handleCallback(bot, callbackQuery, subAction, params, menuManager) {
        ...
        switch (subAction) {
            case 'menu':
                await this.showFortuneMenu(bot, chatId, messageId, userName, menuManager);
                break;
            case 'general':
    ...
    async showTarot(bot, chatId, messageId, userId) {
        ...
        if (messageId) {
            await this.editMessage(bot, chatId, messageId, `🃏 **오늘의 타로카드**\n\n${tarot}`, {
                parse_mode: 'Markdown',
                reply_markup: keyboard
            });
        } else {
            await this.sendMessage(bot, chatId, `🃏 **오늘의 타로카드**\n\n${tarot}`, {
                parse_mode: 'Markdown',
                reply_markup: keyboard
            });
        }
