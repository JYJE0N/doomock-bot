...
class InsightModule extends BaseModule {
    ...
    async handleCallback(bot, callbackQuery) {
        ...
        switch (action) {
            ...
            case 'refresh':
                return await this.showFullInsight(bot, chatId, messageId, from);
            default:
                return await this.sendMessage(bot, chatId, '❌ 알 수 없는 인사이트 명령입니다.');
        }
    }

    async handleInsightCommand(bot, msg) {
        const { text, chat: { id: chatId }, from } = msg;
    ...
    async showFullInsight(bot, chatId, messageId, from) {
        try {
            ...
            await this.sendMessage(bot, chatId, `❌ 인사이트 생성 실패: ${error.message}`);
        }
    }
    ...
    async showProductStrategy(bot, chatId, messageId, from) {
        const strategy = await this.insightService.generateProductStrategy(getUserName(from));
    ...
    async showPricingStrategy(bot, chatId, messageId, from) {
        const strategy = await this.insightService.generatePricingStrategy(getUserName(from));
        await this.editMessage(bot, chatId, messageId, strategy, {
            parse_mode: 'Markdown'
        });
    }
    ...
    async showInventoryStrategy(bot, chatId, messageId, from) {
        const strategy = await this.insightService.generateInventoryStrategy(getUserName(from));
        await this.editMessage(bot, chatId, messageId, strategy, {
            parse_mode: 'Markdown'
        });
    }
    ...
    async showMarketingStrategy(bot, chatId, messageId, from) {
        const strategy = await this.insightService.generateMarketingStrategy(getUserName(from));
        await this.editMessage(bot, chatId, messageId, strategy, {
            parse_mode: 'Markdown'
        });
    }
    ...
    async showRegionalStrategy(bot, chatId, messageId, from) {
        const strategy = await this.insightService.generateRegionalStrategy(getUserName(from));
        await this.editMessage(bot, chatId, messageId, strategy, {
            parse_mode: 'Markdown'
        });
    }
    ...
    async showCompetitorStrategy(bot, chatId, messageId, from) {
        const strategy = await this.insightService.generateCompetitorStrategy(getUserName(from));
        await this.editMessage(bot, chatId, messageId, strategy, {
            parse_mode: 'Markdown'
        });
    }
    ...
    async showNationalStatus(bot, chatId, messageId, from) {
        const status = await this.insightService.generateNationalStatus(getUserName(from));
