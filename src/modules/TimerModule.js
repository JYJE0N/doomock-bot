...
class TimerModule extends BaseModule {
  ...
  async handleCallback(bot, callbackQuery, subAction, params, menuManager) {
    ...
    try {
      switch (subAction) {
        case "menu":
          await this.showTimerMenu(bot, chatId, messageId, userName, userId);
          break;
        case "pomodoro_start":
          await this.showPomodoroTaskPrompt(bot, chatId, messageId, userId);
          break;
        case "pomodoro_quick":
          await this.startQuickPomodoro(bot, chatId, messageId, userId);
          break;
        case "start_prompt":
        ...
        case "complete":
          ...
          break;
        case "stats":
          await this.showUserStats(bot, chatId, messageId, userId);
          break;
        case "continue":
          await this.continuePomodoro(bot, chatId, messageId, userId);
          break;
        case "help":
          await this.showTimerHelp(bot, chatId, messageId);
          break;
        default:
    ...
    const keyboard = {
      inline_keyboard: [
        [
          ...
          { text: "⚡ 빠른 포모도로", callback_data: "timer_pomodoro_quick" },
        ],
        [
          { text: "⏱️ 일반 타이머", callback_data: "timer_start_prompt" },
          { text: "📊 내 통계", callback_data: "timer_stats" },
        ],
      ],
    };

    // 활성 타이머가 있으면 관련 버튼 추가
    if (activeTimer.success) {
  ...
  async completePomodoro(bot, chatId, messageId, userId) {
    const result = this.timerService.completePomodoro(userId, true); // 자동 전환

    if (result.success) {
      this.stopAutoRefresh(userId); // 자동 새로고침 중지

      const data = result.data;
      const isBreak = data.nextMode === "break";
      const emoji = isBreak ? "🎉" : "💪";
      const nextEmoji = isBreak ? "☕" : "💼";

      // ⭐ 성취감을 주는 완료 메시지
      let completionMessage = `${emoji} **${data.message}**\n\n`;

      completionMessage += `✅ **완료**: ${data.completedTask}\n`;
      completionMessage += `⏱️ **소요 시간**: ${this.timerService.formatElapsedTime(
        data.actualDuration
      )}\n`;

      if (data.actualDuration !== data.plannedDuration) {
        const diff = data.actualDuration - data.plannedDuration;
        completionMessage += `📊 **계획 대비**: ${
          diff > 0 ? "+" : ""
        }${diff}분\n`;
      }

      completionMessage += `🎯 **총 세션**: ${data.sessionCount}회\n`;
      completionMessage += `💪 **총 작업 시간**: ${this.timerService.formatElapsedTime(
        data.totalWorkTime
      )}\n\n`;

      if (data.autoTransition) {
        completionMessage += `${nextEmoji} **다음 단계**: ${
          data.nextMode === "work" ? "작업" : "휴식"
        } (${data.nextDuration}분)\n`;
        completionMessage += `⏰ **완료 예정**: ${data.nextCompletionTime}\n\n`;
        completionMessage += `🚀 **자동으로 다음 단계가 시작되었습니다!**`;
      }
      ...
      const keyboard = {
        inline_keyboard: [
          ...
          [
            { text: "⏹️ 세션 종료", callback_data: "timer_stop" },
            { text: "🔙 타이머 메뉴", callback_data: "timer_menu" },
          ],
        ],
      };

      await this.editMessage(bot, chatId, messageId, completionMessage, {
        parse_mode: "Markdown",
        reply_markup: keyboard,
      });
